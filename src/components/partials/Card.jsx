...
const Card = ({data,title}) => {
  return (
    <div className='w-full h-full  p-[3%] flex flex-wrap gap-10'>    
        {data.map((item,index) => (
            <Link to={`/${item.media_type || title }/details/${item.id}`} key={index} className='w-[20vw] h-[40vh] my-7 relative hover:scale-125 transition-transform duration-300 ease-in-out hover:z-10'>
            ...
            <h1 className='text-2xl my-2 text-zinc-300 px-2 font-semibold'>
                ...
                {item.vote_average > 0 && <div className='flex justify-center items-center absolute right-[-10%] bottom-[15%] w-[4vw] h-[7vh] font-bold text-zinc-800 rounded-[50%] bg-yellow-400'>

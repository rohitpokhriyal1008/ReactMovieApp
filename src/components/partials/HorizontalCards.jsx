...
const HorizontalCards = ({data}) => {
  ...
  return (
     ...
     <div className='w-full h-[40vh] flex overflow-x-auto p-2'> 
         { data.map((item,index) => (
            <Link to={`/${item.media_type}/details/${item.id}`} key={index} className='min-w-[20%] mr-3 mb-2  bg-zinc-900  rounded-md overflow-hidden hover:scale-105 shadow-md transition-transform duration-300 ease-in-out'>
